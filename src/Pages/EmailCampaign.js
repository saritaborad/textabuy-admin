...
export default function EmailCampaign() {
			...
			options: {
				...
				customBodyRender: (data, i) => {
					...
					tempArr = data.map((item) => {
						return (item ? item.customergroup.no_of_customer : "-");
					});
				},
			},
		},
	...
	useEffect(() => {
		...
		getAllEmailCampaign(status, option, location?.state?.id);
	}, []);

	const getAllEmailCampaign = (status, option, id) => {
		let data = { status: status ? status : "all", id: id, option: option };
		const getAllEmailCampaignPromise = new Promise((resolve, reject) => {
			resolve(PostApi(API_Path.getEmailCampaignVendorwise, data));
		});
		getAllEmailCampaignPromise.then((res) => {
			if (res.status === 200) {
				let tempArr = [];
				tempArr = res.data.data?.emailcampign?.map((element) => {
					return element;
				});
				setAllEmailCampaignData(tempArr);
	...
	const getstatuswiseData = () => {
		const getstatuswiseDataPromise = new Promise((resolve, reject) => {
			resolve(PostApi(API_Path.export_emailcampaign, { status: status, userid: id }));
		});
	...
	let tableCallBack = (option) => {
		setoption(option);
		getAllEmailCampaign(status, option, id);
	};

	const handleStatusWise = (e) => {
		setstatus(e.target.value);
		getAllEmailCampaign(e.target.value, option, id)
	}
