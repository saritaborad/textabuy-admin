...
export default function TextCampaignVendors() {
	...
	const [columns, setcolumns] = useState([
		...
		{
			label: "No. of TextCampaign",
			value: "textcampaign",
			options: {
				filter: true,
				sort: false,
				customBodyRender: (data, i) => {
					return (<span>{data[i].textcampaign ? data[i].textcampaign : "-"}</span>)
				},
			},
		},
		{
					...
					return (
						<div className="table-ed-drop">
							<Dropdown drop="left">
								...
								<Dropdown.Menu>
									<Dropdown.Item onClick={() => Navigate("/text-campaign", { state: { id: data[i].id } })}>
	...
	const getVendorListData = (option, status) => {
		let data = { status: status, option: option };
					...
					<div className="row">
						...
						<div className="col-md-7 col-sm-8 text-camp-rgt">
							<div className="text-campaign-select my-2">
								<select className="form-select comn-input-style" onChange={(e) => handleStatusWise(e)}>
							...
							<div className="crt-campaign-btn crt-in-btn  my-2" onClick={() => getstatuswiseData()}>
								<a className="btn-comn-class w-100" id="csv" href={vendorListData !== undefined && `data:text/csv;charset=utf-8,${escape(DataInCSV)}`} download="TextCampaignList.csv">Export</a>
							</div>
						</div>
