...
export default function EditPlan() {
	...
	const [inputListUpdate, setinputListUpdate] = useState([{ feature: "" }]);
	const [getinputList3, setgetinputList3] = useState([{ feature: "" }]);

	useEffect(() => {
		setid(location?.state?.id);
		getIdPlan(location?.state?.id);
	}, [location?.state?.id]);

	const getIdPlan = (_id) => {
		const addPlanPromise = new Promise((resolve, reject) => {
			resolve(PostApi(API_Path.getByIdPlan, { planid: _id }));
		});

		addPlanPromise.then((res) => {
			if (res.status === 200) {
				setgetPlanData(res.data.data);
				setmostpopular(res.data.data.mostpopular);
				let getinputList2 = [];
				getinputList2 = res.data.data.features.map((str, index) => ({ feature: str }));
				setgetinputList3(getinputList2);
				setinputListUpdate(res.data.data.features.map((feature) => feature));
			} else {
	...
	const handleMostpopular = (e) => {
		runforms.current.setFieldValue("mostpopular", e.target.checked);
	};

	const ChangePlan = (formData, resetForm) => {
		const addPlanPromise = new Promise((resolve, reject) => {
			resolve(PostApi(API_Path.editplan, formData));
		});
		addPlanPromise.then((res) => {
			if (res.status === 200) {
				Navigate("/settings");
	...
	const handleRemoveFeature = (id) => {
		...
		setgetinputList3(FeatureFilterArray);
	};

	const handleInputChange = (e, index) => {
		const list = [...getinputList3];
		list[index][e.target.name] = e.target.value;
		setgetinputList3(list);
	...
	const handleAddClick = () => {
		setgetinputList3([...getinputList3, {feature: ""}]);
	};
					...
					<div className="row">
						<div className="col-12">
							<div className="comn-title-main">
								<h1 className="mb-0">Edit Plan</h1>
							</div>
						</div>
						<div className="col-12">
							<div className="detail-box-class">
								<Formik
									...
									initialValues={{
										_id: id,
										status: "true",
										planName: getPlanData ? getPlanData.planName : "",
										monthprice: getPlanData ? getPlanData.monthprice : "",
										description: getPlanData ? getPlanData.description : "",
										mostpopular: getPlanData.mostpopular,
									...
									onSubmit={(formData, { resetForm }) => ChangePlan(formData, resetForm)}
								>
									{(runform) => (
										<form onSubmit={runform.handleSubmit} className="row">
											...
											<div className="mb-3 col-12">
												...
												<div className="App">
													{getinputList3.length > 0 && getinputList3.map((x, i) => {
														return (
															<div className="d-flex">
																...
																<div className="btn-box d-flex">
																	{getinputList3.length !== 1 && (
																		<button className="border-0 bg-transparent" type="button" onClick={() => handleRemoveClick(i)}>
																			<img src={Remove} alt="" />
																		</button>
																	)}
																	{getinputList3.length - 1 === i && (
																		<button className="border-0 bg-transparent ml-3" type="button" onClick={handleAddClick}>
											...
											<div className="mb-3 col-12">
												...
												<div className="form-check ms-auto form-switch me-1">
													<input className="form-check-input" type="checkbox" id="ActiveStatus" name="mostpopular" checked={mostpopular} onChange={handleMostpopular} />
												</div>
											</div>
