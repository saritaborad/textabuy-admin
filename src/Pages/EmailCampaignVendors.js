...
export default function EmailCampaignVendors() {
	...
	const columns = [
		...
		{
			label: "No. of EmailCampaign",
			value: "emailcampaign",
			options: {
				filter: true,
				customBodyRender: (data, i) => {
					return <span>{data[i].emailcampaign ? data[i].emailcampaign : "-"}</span>
				},
					...
					return (
						<div className="table-ed-drop">
							<Dropdown drop="left">
								...
								<Dropdown.Menu>
									<Dropdown.Item onClick={() => Navigate("/email-campaign/", { state: { id: data[i]._id } })}>
					...
					<div className="row">
						...
						<div className="col-md-7 col-sm-8 text-camp-rgt">
							...
							<div className="crt-campaign-btn crt-in-btn  my-2" onClick={() => getstatuswiseData()}>
								<a className="btn-comn-class w-100" id="csv" href={vendorListData !== undefined &&`data:text/csv;charset=utf-8,${escape(DataInCSV)}`} download="EmailCampaignList.csv">Export</a>
							</div>
						</div>
